...
void solve() {
	...
	for(int p=1;p<=n;p++) {
		aut[p] = aut[pi[p-1]];
		if(p<n) aut[p][s[p]-'a'] = p+1;
	}
	trace(pi);
	trace(aut);
	// string doub = t+t;
	// vector<vector<vector<mint>>> sum(m+1,vector<vector<mint>>(m+1,vector<mint>(LOGK+1,0))),
	// 						   num(m+1,vector<vector<mint>>(m+1,vector<mint>(LOGK+1,0))); //start at i, end at j, num of copies = 2^k. sum of scores and num of such strings
	// vector<vector<int>> resultant(m,vector<int>(n+1,0)),cnt(m,vector<int>(n+1,0));
	// for(int i=0;i<m;i++) {
	// 	for(int p=0;p<=n;p++) {
	// 		resultant[i][p] = aut[p][s[i]-'a'];
	// 	}
	// }
	// for(int power=0;(1<<power)<=m;power++) {
